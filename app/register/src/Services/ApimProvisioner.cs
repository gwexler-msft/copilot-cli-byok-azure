...
public sealed class ApimProvisioner : IApimProvisioner
{
        ...
        string objectId, string userPrincipalName, string productId, CancellationToken ct = default)
    {
        ...
        // PrimaryKey is nullable on the SDK type: APIM can return a subscription in a transitional
        // state with no key yet. Handing that straight back produced {primaryKey: null} with HTTP
        // 200, and the installer scripts embedded a null key silently (#89). Fail loudly instead.
        var primaryKey = secrets.Value.PrimaryKey
            ?? throw new InvalidOperationException($"APIM returned no primary key for subscription '{sid}'.");

        return new ByokSubscriptionResult(sid, productId, primaryKey);
    }
    ...
    public async Task<string> RegeneratePrimaryKeyAsync(string objectId, CancellationToken ct = default)
    {
        ...
        return secrets.Value.PrimaryKey
            ?? throw new InvalidOperationException($"APIM returned no primary key after regenerating '{ComputeSid(objectId)}'.");
    }
