...
public sealed class IdentityContext
{
    ...
    // Entra's documented value for the logout_hint parameter, which suppresses the "pick an account
    // to sign out of" prompt. Emitted only when the login_hint OPTIONAL CLAIM is enabled on the app
    // registration (setup-register-entra does that). Docs are explicit that a UPN must NOT be
    // substituted here, so absence means no hint - never fall back to the UPN.
    ...
    /// <summary>
    /// Value of the <c>login_hint</c> optional claim, for use as <c>logout_hint</c> on the Entra
    /// end-session endpoint. Null when the claim is not configured on the app registration.
    /// </summary>
    ...
    private void EnsureParsed()
    {
        ...
        try
        {
            ...
            foreach (var claim in claims.EnumerateArray())
            {
                ...
                else if (_loginHint is null && string.Equals(typ, LoginHintClaimType, StringComparison.OrdinalIgnoreCase))
                {
                    _loginHint = val;
                }
