...
set_cli_env() {
  ...
  awk 'BEGIN{skip=0} /^# >>> BYOK >>>$/{skip=1} skip==0{print} /^# <<< BYOK <<<$/{skip=0}' "$profile" > "$tmp"
  {
    ...
    echo "export COPILOT_MODEL='$CLI_MODEL'"
    echo 'export COPILOT_PROVIDER_MAX_PROMPT_TOKENS=1050000'
    echo 'export COPILOT_PROVIDER_MAX_OUTPUT_TOKENS=128000'
    echo '# <<< BYOK <<<'
  } > "$profile"
  rm -f "$tmp"
...
# Reports only. Missing CLI tooling must not block the VS Code half of the install.
show_prereqs() {
  echo 'Prerequisites:'
  if command -v node >/dev/null 2>&1; then
    echo "  Node.js     : $(node --version)"
  else
    echo '  Node.js     : NOT FOUND - needed only by the Copilot CLI'
    echo '                install Node.js 22+ from https://nodejs.org/en/download'
  fi
  if command -v copilot >/dev/null 2>&1; then
    echo "  Copilot CLI : $(command -v copilot)"
  else
