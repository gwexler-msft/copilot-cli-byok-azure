...
blocks** (same key, same APIM, different `apiType` and matching `url`). The shipped Foundry
sample retains an empty legacy Chat Completions provider and puts GPT-5.6 models under Responses.

| File | What's inside |
|---|---|
| [`chatLanguageModels.foundry.json`](chatLanguageModels.foundry.json) | Empty legacy chat provider plus Responses provider, Foundry backend (`/openai/v1/...`), models `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna` |
...
provider block can either be Chat Completions or Responses — not both. GPT-5.6 agent workflows
use Responses. Keep the legacy provider only for older models that still need Chat Completions,
with identical `apiKey` but a matching `apiType` and `url`.

## GPT-5.6 requires Responses for VS Code agents

GPT-5.6 and later models support Chat Completions and function tools separately, but Chat
Completions cannot combine tools with reasoning. VS Code agent requests include function
tools, and GPT-5.6 defaults to `reasoning_effort: "medium"`, so putting a `gpt-5.6-*` model
in the Chat Completions provider fails with `Function tools with reasoning_effort are not
supported`. This is a Foundry model/API constraint, not an APIM routing failure.

Put GPT-5.6 models in the **Responses** provider and use the `/openai/v1/responses` URL.
The register app now does this automatically for commercial model IDs beginning with
GPT-5.6 or a later numeric GPT version. A manual model entry should include:

```json
{
   "id": "gpt-5.6-terra",
   "name": "BYOK gpt-5.6-terra",
...
Foundry also documents a Chat Completions fallback: send top-level
`"reasoning_effort": "none"` on every request containing tools. In VS Code, `none` must
be present in `supportsReasoningEffort` for the Thinking Effort picker to emit it, and
`reasoningEffortFormat` must be `"chat-completions"`; otherwise VS Code drops the picker
value or emits the Responses-shaped field and the backend still uses its `medium` default.
Responses is preferred because it keeps reasoning available during tool calls.
...
**Mismatching them sends the wrong body shape**; the backend can ignore or reject it and
then run at its default effort. Match each model's surface (its `url`) to the matching
...
5. Substitute `<APIM_HOSTNAME>` (e.g. `apim-byok-gov-pilot-<suffix>.azure-api.us`) and
...
7. **Set the key through the UI as well**, even though you just put it in the file: run
   **`Chat: Manage Language Models`**, select the gear icon beside the provider, and paste the
   subscription key. VS Code keeps a per-provider key in **secret storage** and sends that in
   preference to the `apiKey` in the file, so a key left over from an earlier setup silently wins
   and the gateway rejects it with *"Access denied due to invalid subscription key"* — a config
   file that looks correct does not rule this out. Note the wording: *invalid* means a key **was**
   sent and matched nothing; *missing* means none was sent at all.

   > Don't reach for `"apiKey": "${input:...}"` here. VS Code's docs recommend it, but it does not
   > resolve in `chatLanguageModels.json` — verified 2026-08-07: no prompt appeared and the literal
   > token was sent to the gateway. See issue #129.
8. Open a new chat (Ctrl+Alt+I), pick a `BYOK …` model from the picker, and ask
