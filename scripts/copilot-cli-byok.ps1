...
# commercial route), so append /openai only if the dev passed a bare host with neither suffix (and
# tolerate a trailing slash). This way a forgotten suffix can't silently break things later.
$ApimBaseUrl = $ApimBaseUrl.TrimEnd('/')
if ($ApimBaseUrl -notmatch '(?i)/openai(-commercial)?$') { $ApimBaseUrl = "$ApimBaseUrl/openai" }
