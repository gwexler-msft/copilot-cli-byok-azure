...
# login_hint is what the sign-out flow passes as logout_hint to skip the "pick an account to sign
# out of" prompt (#136). Entra only emits it when requested as an optional claim.
echo "Configuring redirect URI, v2 tokens, and claims..."
PATCH_FILE="$(mktemp)"
cat > "$PATCH_FILE" <<EOF
{
  "web": {
...
if [[ -n "$HAVE_SECRET" && -z "$ROTATE" ]]; then
  ...
  # Two DIFFERENT failure modes, and conflating them cost real debugging time:
  ...
  #                         RBAC or waiting fixes it. Retrying just burns 60s and then reports
  #                         "role not propagated", which is simply untrue.
  ...
  NET_BLOCKED=
  grep -qiE 'public network access is disabled|private link|not from a trusted service|ForbiddenByFirewall|ForbiddenByConnection' /tmp/kvset.err && NET_BLOCKED=1

  if [[ -z "$WRITTEN" && -z "$NET_BLOCKED" ]]; then
    if grep -qiE 'Forbidden|secrets set permission|AuthorizationFailed|403' /tmp/kvset.err; then
      ...
      for i in 1 2 3 4 5 6; do
        sleep 10
        if az keyvault secret set --vault-name "$KEYVAULT_NAME" --name "$SECRET_NAME" --value "$SECRET_VALUE" -o none 2>/dev/null; then WRITTEN=1; break; fi
  ...
  # ARM fallback: a Microsoft.KeyVault/vaults/secrets DEPLOYMENT is a control-plane call and ARM
  # is a trusted service, so the secret lands while the vault stays locked. secretValue is a
  # securestring param, so ARM records it as null in deployment history. Mirrors
  # scripts/rotate-runner-pat-breakglass.ps1.
  if [[ -z "$WRITTEN" ]]; then
    if [[ -n "$NET_BLOCKED" ]]; then
      echo "Vault data plane is closed to the public network - writing via ARM deployment (vault stays locked)..."
    else
      echo "Data-plane write still denied - trying the ARM deployment path..."
    fi
    KV_RG="$(az keyvault show --name "$KEYVAULT_NAME" --query resourceGroup -o tsv 2>/dev/null || true)"
    TMPL="$(mktemp /tmp/easyauth-kvsecret-XXXXXX.json)"
    cat > "$TMPL" <<'JSON'
{
...
JSON
    if [[ -n "$KV_RG" ]] && az deployment group create -g "$KV_RG" \
        --name "easyauth-secret-$(date -u +%Y%m%d%H%M%S)" --template-file "$TMPL" \
        --parameters vaultName="$KEYVAULT_NAME" secretName="$SECRET_NAME" secretValue="$SECRET_VALUE" \
  ...
  if [[ -z "$WRITTEN" ]]; then
    manual_fallback "could not write the client secret to Key Vault (data plane AND ARM deployment both failed)."
    exit 0
  fi
