...
<#
.SYNOPSIS
  COMMERCIAL-tenant peer setup for the cross-cloud commercial Foundry BACKEND. Creates the secretless
...
.EXAMPLE
  ./setup-commercial-backend.ps1 -FoundryAccountName aifcopilotbyokcommpilot<suffix> `
    -FoundryResourceGroup rg-copilot-byok-comm-pilot `
    -GovTenantId <TENANT_ID> `
    -GovApimMiObjectId <OBJECT_ID> `
    -GovEgressIp <NAT_EGRESS_IP> -AllowGovEgressIp
#>
[CmdletBinding()]
param(
