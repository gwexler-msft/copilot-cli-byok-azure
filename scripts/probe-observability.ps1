...
function Invoke-Kql([string] $endpoint, [string] $token, [string] $query) {
    ...
    # The leading comma matters: without it a single-row result is unrolled into its own columns,
    # and $_[0] then indexes a character instead of a field.
    if ($resp.tables -and $resp.tables[0].rows) { return ,$resp.tables[0].rows } else { return @() }
}
