...
GitHub Actions is the recommended deployment path for pilots and shared environments. Install
these tools on an operator workstation for first-run bootstrap, validation, and recovery; the
self-hosted runner image carries the deployment toolchain used by CI.

| Tool | Min version | Why it's needed | Install / upgrade |
|---|---|---|---|
| **Azure Developer CLI (`azd`)** | **1.25.4** | Local bootstrap/recovery driver and the provisioning engine used by CI. Reads [azure.yaml](../azure.yaml) and provisions `infra/main`. | `winget install Microsoft.Azd` / `winget upgrade Microsoft.Azd` (or `brew install azd`, `curl -fsSL https://aka.ms/install-azd.sh \| bash`) |
...
If it builds clean, preview the plan. As a note, no output = clean build. This command only prints anything when there are warnings or errors. With the local `azd` path this is `azd provision
...
**Use GitHub Actions for pilots and shared environments.** Complete the
[CI/CD one-time setup](#one-time-setup), then follow the
[end-to-end deployment sequence](#end-to-end-deployment-sequence). This prevents a local
`azd provision` with an incomplete environment from replacing parameterized named values or
secrets with placeholders. Use the local flow below only to bootstrap the first pilot runner,
for an isolated developer environment, or for recovery.

### Local bootstrap / recovery path

[azure.yaml](../azure.yaml) wires `azd` to the same
...
> **Model/SKU:** the current Commercial and Gov profiles deploy **gpt-5.6-sol
> (2026-07-09) on DataZoneStandard** as the primary model. They also deploy
> **gpt-5.6-terra (2026-07-09)** as an additional explicit model.
>
> **Mini tier (auto-routing):** `deployMiniModel=true` also deploys a cheap tier on each
> backend — **gpt-5.6-luna (2026-07-09) on DataZoneStandard** — used when a caller sends
> the sentinel model `auto`. Confirm the exact name, version, SKU, and quota in the target
> subscription and region before deploying. Tune routing via the `autoRoute*` params
> (threshold 500, band 200, classifier off by default). Set `deployMiniModel=false` to skip
> the tier entirely.
>
...
> `2025-04-01-preview`). Deployment-scoped GPT-5 requests need this version or later; an older
...
>
> **Completion-token field:** when probing Chat Completions, use `max_completion_tokens`.
> This is accepted across the Sol/Luna auto-routing pair and avoids the `max_tokens` rejection
> returned by reasoning models.
...
- **Model migrations need an explicit cleanup step.** The GPT-5.6 rollout adds Sol, Luna, and
  Terra, but incremental mode leaves old GPT-5.1 and mini deployment children in place. Verify
  explicit and `auto` Responses calls against the new models, then delete only the retired model
  deployments in a separately approved operation.
...
### (Optional) Enable the Commercial Foundry backend

An opt-in **extra backend** that lets a **Gov** gateway reach a **Commercial** Microsoft Foundry
endpoint over the public internet. There is **no second base URL**: the default `/openai` route
selects the commercial backend **per request** when the requested model matches the
`commercialModels` sentinel, so clients are unchanged. **Validated 2026-07-01** (gov-dev →
commercial pilot Foundry returned HTTP 200 for both chat and responses). See
[commercial-foundry-route.md](commercial-foundry-route.md) for the full design, request flow,
parameters, and validation checklist.
...
2. **Gov deployment (parameters + re-provision)** — set `deployFoundryCommercial=true`,
   **`commercialModels`** (the comma-separated model names/prefixes the `/openai` route should send
   to the commercial backend — leave it empty and nothing ever routes there),
   ...
   `deployFoundryCommercial`; models outside `commercialModels` still go to the private Foundry.

> **Anthropic/Claude models** are commercial-hosted and selected by the same sentinel, but they
> speak the native Anthropic Messages wire format and send their key as `x-api-key` — so they also
> need the opt-in `/anthropic` route (`deployAnthropicRoute=true`). Asking for one on `/openai`
> returns a typed `400 WireFormatMismatch` naming that fix; the gateway does not reshape requests.

**To turn it back off:** set `deployFoundryCommercial=false` (or just empty `commercialModels` to
stop routing without removing anything) and re-provision — ARM incremental removes the commercial
backend/named-values/egress rule. The commercial-tenant SP + role + firewall rule are independent of
the Gov deployment — remove them in the commercial tenant if no longer needed.
...
|---|---|---|
| **Microsoft Visual C++ 2015–2022 Redistributable (x64)** | Yes (Windows) | Required by the CLI's `cli-native.node`. On stock Windows Server, `copilot --version` can work while interactive `copilot` exits immediately. The debug log says `Native addon "cli-native" not found` / `The specified module could not be found`. Install `Microsoft.VCRedist.2015+.x64`. |
...
#### Provider type ⇄ wire format (verified by wire capture)

`COPILOT_PROVIDER_TYPE` decides three things at once: **what the client does with the path on your
base URL**, **which header carries the credential**, and **which body shape it sends**. Getting this
wrong is the single most common BYOK misconfiguration, and the failure is confusing — the request
either 404s at a path you never configured or 401s before any policy runs.

| `COPILOT_PROVIDER_TYPE` | Path on the base URL | Request actually sent | Credential header | Body |
|---|---|---|---|---|
| `azure` + `COPILOT_PROVIDER_WIRE_API=responses` (wrapper default) | **DISCARDED** | `<origin>/openai/v1/responses` | `api-key` | Responses |
| `azure` + `COPILOT_PROVIDER_WIRE_API=completions` | **DISCARDED** | `<origin>/openai/v1/chat/completions` | `api-key` | OpenAI chat/completions |
| `openai` | preserved | `<base>/chat/completions` | `Authorization: Bearer` | OpenAI chat/completions |
| `anthropic` | preserved | `<base>/v1/messages` | `x-api-key` | Anthropic Messages |

Consequences worth internalising:

- **`azure` can only ever reach `/openai`.** It rewrites the path to `/openai/v1/...` no matter what
  you put on `COPILOT_PROVIDER_BASE_URL`, so a gateway route on any other path is unreachable from
  this provider type. That is why commercial models are selected by the `commercial-models`
  **sentinel on `/openai`** rather than by a separate route.
- **`openai` needs `/v1` in the base URL.** It appends `/chat/completions` with no version segment,
  so `https://<gateway>/openai` gets you a 404 — use `https://<gateway>/openai/v1`.
- **`anthropic` sends `x-api-key`.** APIM validates the subscription key from the header declared on
  the API *before any policy executes*, and an API can declare only one such header — which is why
  `/anthropic` is a separate APIM API rather than an extra operation on `/openai`. Set
  `COPILOT_PROVIDER_BEARER_TOKEN` instead to send `Authorization: Bearer` (jwt mode).
...
| Visual C++ runtime | **2015–2022 x64** | supplies the native runtime used by `cli-native.node`; stock Windows Server may not include it | `winget install Microsoft.VCRedist.2015+.x64` |
...
winget install Microsoft.VCRedist.2015+.x64 # native addon runtime (Windows Server)
winget install GitHub.Copilot            # the Copilot CLI

# OR all platforms (npm — needs Node 22+ already installed), pin a recent build:
npm install -g @github/copilot@latest

# Verify (expect 1.0.54 or higher; validated with 1.0.82):
copilot --version
```
...
> request can't overflow whichever tier it lands on: **1050000** prompt and **128000** output.
> Override either with `-MaxPromptTokens` / `-MaxOutputTokens`. A named catalog model (for
> example `gpt-5.6-sol`) leaves these unset and uses the
...
#                                  -ApimBaseUrl 'https://apim-...azure-api.us/openai' -Model gpt-5.6-luna -Test
```

The wrappers default to `-WireApi responses`, which Copilot CLI 1.0.82 supports and GPT-5.6
agent tool calls require. Commercial and Gov deploy Sol, Luna, and Terra natively; `auto`
...
> suffix is appended if omitted, `WIRE_API` defaults to **`responses`**, `[model]` defaults to
> **`auto`**, and for a non-catalog model it
> exports the same token limits (`COPILOT_PROVIDER_MAX_PROMPT_TOKENS=1050000` /
> `COPILOT_PROVIDER_MAX_OUTPUT_TOKENS=128000`, overridable via the `MAX_PROMPT_TOKENS` /
...
> **GPT-5.6 and later models belong on the Responses provider for VS Code agents.** Agent
> requests contain function tools, while GPT-5.6 Chat Completions rejects tools whenever
> reasoning is enabled or omitted (the omitted default is `medium`). Configure `apiType:
> "responses"`, a `/v1/responses` URL, and `reasoningEffortFormat: "responses"`. The
> register app applies this rule automatically. The Chat Completions workaround is an
> explicitly emitted top-level `reasoning_effort: "none"`, but it disables reasoning and
> is not the recommended VS Code configuration.
...
…then replace `<APIM_HOSTNAME>` and `<APIM_SUBSCRIPTION_KEY>` everywhere they appear and save.

> **The key goes in the file, and you should also set it in the UI.** VS Code keeps a per-provider
> key in **secret storage** and sends that in preference to the `apiKey` in the file, so a key left
> from an earlier setup silently wins and APIM answers *"invalid subscription key"* against a
> correct-looking config (#129). Set it via *Chat: Manage Language Models* → gear icon.
> VS Code's documented `"apiKey": "${input:...}"` form does **not** resolve in
> `chatLanguageModels.json` — verified 2026-08-07: no prompt appeared and the literal token was sent.
...
```jsonc
// chatLanguageModels.json (legacy provider retained empty; GPT-5.6 uses Responses)
[
  {
...
- **`reasoningEffortFormat`** must match the provider's `apiType`: `"chat-completions"`
  ...
  `reasoning.effort`. A mismatch sends the wrong shape, which the backend can ignore or
  reject before falling back to its model default.
...
"productTiers": { "value": [
  { "name": "byok-standard", "displayName": "BYOK Standard", "description": "Standard tier.",
    "callsPerMinute": 60,  "tokensPerMinute": 100000, "monthlyCallQuota": 50000 },
  { "name": "byok-power",    "displayName": "BYOK Power",    "description": "Power tier.",
    "callsPerMinute": 120, "tokensPerMinute": 200000, "monthlyCallQuota": 200000 }
] }
```

> **Sizing note — a tier TPM is bounded on both sides.**
>
> *Lower bound:* VS Code Copilot Chat packs editor context into every request (typically
> 13-15k tokens), so a 20,000 TPM tier 429s on the second turn within a minute. Keep
> `byok-standard` at 100,000 TPM or above for interactive use.
>
> *Upper bound:* a tier TPM above the environment's `modelCapacity` (TPM units of 1000) means
> APIM stops being the throttle and the **model deployment** becomes it. The dev environments
> run `modelCapacity: 25` (25k TPM, smoke probes only) and so deliberately keep
> `byok-standard` at 20,000; the pilots run `250` (250k TPM) and use 100,000. Raise
> `modelCapacity` before raising a tier.
>
> If you change a tier's TPM, re-check `OversizedTokens` / `OVERSIZED_TOKENS` in the smoke
> tests: assertion #5 proves the throttle by exceeding it, so it reports a false FAIL if the
> burst can't reach the limit — and it now FAILs explicitly if the 429 came from the backend
...
> plus the live VNet-injected self-hosted runners, post-deploy smoke tests, dev lifecycle,
> and daily Project Votes refresh job.

Steps 0–3 above provide the local bootstrap and recovery path. For repeatable, auditable pilot
deployments, use the manual-trigger pipeline at
...
- runs **on demand only** (`workflow_dispatch`) — pick the **cloud**
  ...
  `deploy` = provision);
- authenticates to Azure with **OIDC federated credentials** — no Azure client secret is stored;
- **validates** first (Bicep build/lint + `azd provision --preview` what-if), then
- **deploys** with `azd provision` (same [azure.yaml](../azure.yaml) hooks as local), gated by
  explicit manual dispatch. Add GitHub Environment reviewers when the repository plan supports
  them; otherwise the operator starting `action=deploy` is the approval gate.
...
> over `infra/main.parameters.json`, then azd substitutes tenant-specific values from GitHub
> Environment variables at run time. **Nothing tenant-specific or secret is
...
az ad app federated-credential create --id $appId --parameters '{
  ...
  "subject": "repo:<OWNER>/<REPO>:environment:gov-pilot",
...
> Add one federated credential **per environment** you deploy (`comm-pilot`, `gov-pilot`,
> `comm-dev`, and `gov-dev`). Prefer
> [`scripts/setup-deploy-app-fics.ps1`](../scripts/setup-deploy-app-fics.ps1) or its bash twin;
> the `subject` must exactly match the GitHub Environment used by the job.

**2. Create the GitHub Environments.** In **Settings → Environments**, create `gov-pilot`,
`comm-pilot`, `gov-dev`, and `comm-dev`. On each, add **Required reviewers** when supported and the
...
> the same names. Never commit their real values.

**3. Configure non-Azure workflow credentials.** Store `GH_RUNNER_PAT` as a GitHub Actions
secret. In this EMU repository, hosted runners are disabled and GitHub App runner auth is not
viable, so the ACA Job runners use a classic PAT. EMU limits its lifetime to roughly one week;
rotate it before expiry. `PROJECT_TOKEN` is separate and is needed only by the Project Votes
refresh workflow. Never pass either token through chat or commit it.

**4. Bootstrap runner deployment permissions.** The pilot runner UAMI provisions the matching
dev environment. Grant it subscription-scope **Contributor** and **User Access Administrator**
before the first `deploy-dev` run. See
[CI/CD → Runner UAMI subscription-scope RBAC](cicd.md#runner-uami-subscription-scope-rbac-one-time-bootstrap-per-cloud).

### End-to-end deployment sequence

1. **Prepare all four committed CI parameter files.** Use the `ci.commercial`,
  `ci.commercial-dev`, `ci.gov`, and `ci.gov-dev` files. Keep tenant-specific values in GitHub
  Environment variables/secrets, not in the files.
2. **Bootstrap the first pilot runner.** A new cloud cannot run its own workflow yet. Provision
  the first pilot locally using the recovery path above. When the other cloud already has a
  healthy pilot runner, dispatch the target cloud with `runnerLabel=<other-pilot-label>`.
3. **Seed and verify runner auth.** Put the active PAT in the repo secret and pilot runner Key
  Vault. For private-endpoint-only pilot vaults, use
  [`scripts/rotate-runner-pat-breakglass.ps1`](../scripts/rotate-runner-pat-breakglass.ps1),
  which also forces the ACA Job to re-resolve its Key Vault secret reference. Confirm a queued
  workflow causes a fresh runner execution before proceeding.
4. **Build the runner images.** Run `build-runner-image.yml` before setting
  `useAcrRunnerImage=true`; ACA validates the image manifest while provisioning and cannot point
  at an image that has not been pushed yet.
5. **Validate, then deploy each pilot.** Dispatch `deploy.yml` first with `action=validate`,
  inspect the what-if, then dispatch with `action=deploy` for `AzureCloud` and
  `AzureUSGovernment`. Leave `runnerLabel` blank after bootstrap.
6. **Deploy dev and run smoke.** Dispatch `deploy-dev.yml` with
  `envs=comm-dev,gov-dev` and `smoke=true`. Each pilot runner provisions its matching dev
  environment; smoke then runs on the new dev runner inside that VNet.
7. **Review every smoke leg.** Require gateway discovery, chat completions, Responses create,
  stored-response retrieve/input-items/cancel, auth rejection, and telemetry assertions to
  pass. Foundry stored-response reads are eventually consistent, so the smoke scripts use a
  bounded retry rather than treating the first `404 Response ... not found` as routing failure.
8. **Validate the pilot test VM.** Connect through Bastion with the bare local username, install
  PowerShell 7, Node.js 22+, the Visual C++ 2015–2022 x64 runtime, and Copilot CLI, then run the
  wrapper smoke test against the private APIM FQDN.

`azd provision` is incremental, but it still reasserts supplied values. Do not run it locally
against a pilot with an incomplete environment. If APIM reports a transient bad ETag after a
parallel deployment, wait until its provisioning state is `Succeeded`, then rerun only that
environment. Do not overlap provision, teardown, and smoke for the same environment.
