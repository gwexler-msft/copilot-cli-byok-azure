...
> **Model discovery is a separate, restricted surface.** `GET /v1/models` lives at
> `https://<apim>/discovery/v1/models` on a dedicated API gated by the `byok-discovery`
> APIM product. The `dev1` / `dev2` chat keys are *not* members of that product and will
> get `401` if they hit `/discovery` — by design. A `smoke` subscription scoped to
> `byok-discovery` is provisioned automatically (used by CI). To grant a human admin
> the same discovery capability, add another entry to `testSubscriptions` with
> `product: 'byok-discovery'` (see
> [architecture → Model discovery](architecture.md#model-discovery--separate-api-behind-a-restricted-product-61))
> and redeploy, or create a one-off APIM subscription scoped to `/products/byok-discovery`
> out of band.
...
`subscription-key` query param). The cleanest fit is to override the header via a
**per-model** `requestHeaders` block and leave the provider's `apiKey` set to the same
value (VS Code requires the field). `requestHeaders` is a model-level field per the VS
Code schema — a provider-level override is silently ignored, which surfaces as
`Access denied due to missing subscription key` from APIM.
...
and matching `url`. **`requestHeaders` is model-level** per the schema, so it must be
repeated on every model (a provider-scoped `requestHeaders` is silently ignored):
    ...
    "models": [
      {
        "id": "gpt-5.1",
        "name": "BYOK gpt-5.1",
        "url": "https://apim-<env>-<suffix>.azure-api.us/openai/v1/chat/completions",
    ...
    "models": [
      {
        "id": "gpt-5.1",
        "name": "BYOK gpt-5.1",
        "url": "https://apim-<env>-<suffix>.azure-api.us/openai/v1/responses",
        "requestHeaders": { "api-key": "<APIM_SUBSCRIPTION_KEY>" },
        "toolCalling": true,
        "vision": false,
        "thinking": true,
...
- **Provider-level vs model-level.** `name` / `vendor` / `apiType` / `apiKey` live on the
  provider; `id` / `name` / `url` / `requestHeaders` / capability flags live on each
  model. A model's `apiType` (rare override) can also shadow the provider's, but it's
  cleaner to split into a second provider block.
- **`requestHeaders` MUST be at model level.** A provider-level `requestHeaders` is
  silently dropped, so VS Code falls back to `Authorization: Bearer <apiKey>` only and
  APIM responds `Access denied due to missing subscription key`.
- **`url`** is the full endpoint, including the route. The Foundry API at path `openai`
  is shown above; substitute `/aoai/v1/...` for the legacy AOAI API.
...
- `401 Unauthorized` with `Unauthorized. Access token is missing, invalid, ...`: the
  ...
  `authMode=subscriptionKey`, or switch the credential by using the JWT bearer in
  `apiKey` and removing the `requestHeaders` override.
...
**Discovery product (model listing).** The dedicated `byok-discovery` product that fronts
`/discovery/v1/models` is independent from the chat tiers and has its own knobs — defaults
**30 calls/min** and **5,000 calls / 30 days**, with **no** token-cost guard (listing models
has no token cost). Tune via:

```jsonc
"discoveryCallsPerMinute":  { "value": 30 },
"discoveryMonthlyCallQuota": { "value": 5000 }
```
...
| `AZURE_TENANT_ID` | `ec95faea-…` | deploy **and** workload tenant (also used for `entraTenantId`) |
