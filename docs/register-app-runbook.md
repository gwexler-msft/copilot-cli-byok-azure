...
4. **Run the installer once.** It merges the BYOK model config into your VS Code user folder
   ...
   - Windows (PowerShell 7): `Unblock-File .\Use-Byok.ps1` then `pwsh -File .\Use-Byok.ps1`
   - macOS / Linux (bash): `chmod +x use-byok.sh && ./use-byok.sh`

   > **Windows: the `Unblock-File` step is required, not optional.** The browser tags every
   > download with the Mark of the Web, and the default `RemoteSigned` execution policy refuses
   > to run a downloaded script that is not digitally signed:
   > `Use-Byok.ps1 cannot be loaded. The file ... is not digitally signed.`
   > `Unblock-File` clears that tag and leaves the machine's execution policy untouched, which is
   > why it is preferred over relaxing the policy. If a machine-wide policy still blocks it, use
   > `pwsh -ExecutionPolicy Bypass -File .\Use-Byok.ps1` — that applies to the single process
   > only. The script declares `#requires -Version 7.0`, so it must run under `pwsh`, not
...
6. **Set the key in the VS Code UI — do this even though it is already in the file.**
   Run **Chat: Manage Language Models**, select the gear icon beside the BYOK provider, and paste
   your subscription key.

> **Why step 6 is not optional.** VS Code keeps a per-provider API key in its own **secret storage**
> and sends that in preference to the `apiKey` in `chatLanguageModels.json`. Anyone who has used the
> provider before, or who regenerated their key, keeps sending the old one no matter what the file
> says. The gateway then answers `Access denied due to invalid subscription key` while the config on
> disk looks perfectly correct, which is why this costs people so much time to work out.
>
> The wording is the diagnostic: APIM says *invalid* when a key **was** sent and matched nothing,
> and *missing* when none was sent. So *invalid* with a correct-looking file means something other
> than the file is supplying the key. APIM accepts both the primary and secondary key, so a
> primary/secondary mix-up is never the explanation.
>
> Microsoft's guidance is to keep the key in secret storage rather than inline
> (`"apiKey": "${input:...}"`) — see
> [Update model provider details](https://code.visualstudio.com/docs/copilot/customization/language-models).
> Whether to render that form instead of an inline key is tracked in issue #129.
> Verified on VS Code 1.131.0.
...
2. **VS Code `settings.json`** — the utility/small-model pair (`chat.utilityModel`,
   `chat.utilitySmallModel` → `BYOK gpt-5.6-luna`) **and** the telemetry/call-home lockdown
