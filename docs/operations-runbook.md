...
| **400** `Unrecognized request argument supplied: snippy` | Copilot-only request extension not stripped (policy drift) | Confirm policy matches repo | redeploy policy (§7) |
...
**First-time bring-up is single-pass (since #86).** `azd provision` with `deployGhRunner=true` and
`ghRunnerSecretFromKeyVault=true` deploys in the order
`runner-identity` → `runner-kv` → `gh-runner`, and `runner-kv` **seeds** the active mode's secret
(`gh-pat` / `gh-app-key`) from the same `ghRunnerPat` / `ghAppPrivateKey` value CI already injects,
so the vault, the secret and the runner UAMI's **Key Vault Secrets User** role all exist before the
Job's `keyVaultUrl` reference is fetch-validated. Notes:

- The seed write goes through **ARM**, a trusted first-party service, so it succeeds even with the
  vault locked to `publicNetworkAccess=Disabled` (`deployRunnerKvPrivateEndpoint=true`).
- An **empty** credential value **skips** the seed instead of blanking it, so a provision run
  without `GH_RUNNER_PAT` in its environment can't wipe a live runner credential.
- On a genuinely fresh env the Job write can still lose a race with Key Vault **RBAC propagation**
  (`Unable to get value ... for secret gh-pat`). `deploy.yml` already retries phase 1 once after
  90s and then runs a phase-2 provision, so it converges without intervention.
- Every later rotation/roll is the vault write above — no re-provision.
...
- **`subscriptionKey` mode:** limits are baked into each **product policy**
  ...
  60 calls/min · 100k TPM · 50k/mo; `byok-power` 120 · 200k · 200k. Change a tier by editing
  `productTiers` and redeploying (the values are inlined into the product policy XML).
  Don't drop `byok-standard` below ~100k TPM: VS Code Copilot Chat sends 13-15k tokens per
  turn, so a 20k ceiling 429s on the second turn within a minute.
...
```pwsh
$job = "caj-proxydns-$env-$suffix"   # e.g. caj-proxydns-gov-pilot-<suffix>
