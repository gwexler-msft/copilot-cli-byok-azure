...
### Bootstrapping a cloud whose pilot runner does not exist yet

`deploy.yml`'s `validate`/`deploy` jobs normally run on the **target cloud's** pilot runner. In a
brand-new or rebuilt tenant that runner does not exist — it is created *by* the deploy that cannot
start — and EMU hosted runners are disabled, so the job queues forever.

The `runnerLabel` dispatch input overrides the label for exactly this case. Leave it blank for
normal runs; set it to the *other* cloud's healthy pilot to bootstrap:

```pwsh
# First Gov deploy into a fresh tenant, hosted on the commercial pilot runner.
gh workflow run deploy.yml -f cloud=AzureUSGovernment -f action=deploy -f runnerLabel=comm-pilot
```

Drop the override once the target pilot runner exists — it hosts its own deploys from then on.
The full tenant-rebuild sequence (Entra apps, FICs, resource-provider registration, ACR image
pre-bake, BYOK groups) is in [deployment-guide.md → "One-time setup"](deployment-guide.md#one-time-setup).
...
1. **Model list reachable** (`#61`) — `GET /openai/v1/models` with the `dev1` APIM
  subscription key returns 200 and lists the expected model deployments (gpt-5.6-sol,
  gpt-5.6-luna, and gpt-5.6-terra). Model listing is an operation on the foundry inference API reachable by any
