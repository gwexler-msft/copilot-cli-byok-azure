...
flowchart LR
    ...
    subgraph CommCloud["Commercial cloud — separate tenant, public internet (opt-in)"]
        COMMFOUNDRY["Commercial Microsoft Foundry<br/>kind=AIServices · public endpoint (NOT a private endpoint)<br/>OPT-IN BACKEND (deployFoundryCommercial=true) — no separate route<br/>picked per-request on /openai (and /anthropic) when the model<br/>matches the commercial-models sentinel — #118<br/>reached over the public internet via the NAT gateway egress IP<br/>cross-tenant service-principal token minted in the Commercial tenant"]
    end
    ...
    APIM -. "deployFoundryCommercial=true: model matches the commercial-models sentinel →<br/>NAT egress (public IP) → cross-tenant SP token (Commercial tenant), leaves the Gov VNet" .-> COMMFOUNDRY
...
> `.vault.usgovcloudapi.net`). Bring-up is **single-pass** (#86): the module order is
> `runner-identity` → `runner-kv` (vault + RBAC + secret **seeded** from the CI-injected credential
> via ARM, which works even with the vault network-locked) → `gh-runner` (the Job whose Key Vault
> reference is fetch-validated on write). `app` is the **module default** and the **recommended path for
...
- **The Commercial Foundry is an opt-in extra _backend_, not an extra route**
  (`deployFoundryCommercial=true`). Clients keep **one** base URL: the default `/openai` route
  selects the commercial backend **per request** when the requested model matches the
  `commercial-models` sentinel (#118), reaching it over the public internet (egress via the NAT
  gateway public IP). The parallel `/openai-commercial` route this replaced is **retired** — see
  [Commercial Foundry backend](commercial-foundry-route.md).
- **`/anthropic` is a second _wire format_, not a second backend** (`deployAnthropicRoute=true`).
  Clients that speak the native Anthropic Messages API (`COPILOT_PROVIDER_TYPE=anthropic`) send
  their credential as `x-api-key`, and APIM validates the subscription key from the header
  **declared on the API before any policy runs** — an API can declare only one such header, so
  those clients need their own front door. Same subscription key, same product tier, same
  quotas and telemetry; the backend is still chosen by the `commercial-models` sentinel.
...
request with the model/deployment in the request **body**, not the URL. The wrapper defaults
`COPILOT_PROVIDER_WIRE_API=responses`; set it to `completions` only for a legacy backend:

```
POST {COPILOT_PROVIDER_BASE_URL}/v1/responses
...
gateway accepts both wire formats on the same hostname and routes them to the selected backend
(see *Responses route* below). GPT-5.6 agent tool calls require Responses.
...
|---|---|---|---|
| **OpenAI-compatible (short)** — our default | `POST /openai/v1/responses` + `{"model":"gpt-5.6-sol"}` | request **body** | yes (`auto` / `byok-auto` sentinel) |
| **Azure-OpenAI-native (legacy / wizard)** | `POST /openai/deployments/gpt-5.6-sol/chat/completions?api-version=…` | **URL** path segment | no (explicit deployment) |
...
`/anthropic` route has **neither** shape — it exposes a single native Anthropic Messages
operation (`POST /v1/messages`), since its clients are configured fresh. See issue #105.
...
| Model location | URL **and** body (must match) | **Body only** — `{ "model": "gpt-5.6-sol", ... }` |
...
- **`stream_options.include_usage` injection**: only applied when the path is **not**
  ...
  and may reject unknown body fields, so standard Responses fields are passed through unchanged.
- **Copilot extension normalization**: removes the top-level `snippy` object that GitHub Copilot
  clients can add. It is a GitHub service extension, not an OpenAI Responses parameter, and native
  Azure OpenAI / Foundry endpoints reject it with `400 Unrecognized request argument supplied:
  snippy`.
- **Token-metric emission**: handled by the built-in `llm-emit-token-metric` policy
  (`Prompt Tokens` / `Completion Tokens` / `Total Tokens`), which understands the usage
  shape of both surfaces. Dashboards and KQL queries don't need to know which wire format
  produced a row.
...
- **Auto model rewrite**: after `auto` / `byok-auto` resolves to Sol or Luna, the policy
  replaces the sentinel in `body.model` with that deployment name. This is mandatory for
  Responses because its account-root backend URL carries no deployment name.
...
### Model API-type discovery (capability map)

Which surface(s) a given model supports (`chat/completions`, `completions`, `embeddings`,
`responses`, Anthropic `messages`) is **discovered**, not hard-coded. `GET /v1/models` exposes
no `responses` capability flag, so the surfaces are **probed**: for each enumerated model,
[`scripts/discover-model-types.ps1`](../scripts/discover-model-types.ps1) (and its `.sh` twin)
sends a minimal request to each candidate surface **through the same gateway route** and records
`200` (supported) vs a typed `4xx` (unsupported). The result is a compact JSON map
`{ "<model>": ["chat/completions","responses", ...] }`, one **per route/cloud**, published to APIM
as a named value:

| Route | Named value | Bicep param |
|---|---|---|
| `/openai` (Foundry) | `foundry-model-types` | `foundryModelTypes` |
| `/aoai` | `aoai-model-types` | `aoaiModelTypes` |
| `/anthropic` (native Messages) | `foundry-commercial-model-types` | `foundryCommercialModelTypes` |

The map is keyed by **route + model** because the same model name can differ across gov vs
commercial (and Claude / commercial-only models are served on one route only). The discovery script
...
- **Telemetry hygiene preserved.** That op policy has an empty outbound (no `<base />`), so it
  ...
  `copilot_byok_request` or the token metrics.
...
|---|---|---|
| **Full** | `gpt-5.6-sol` (DataZoneStandard) | Coding/debugging, long or technical prompts, ambiguous prompts (fail-safe) |
| **Cheap** | `gpt-5.6-luna` (DataZoneStandard) | Short, non-coding, conversational prompts |
...
flowchart TD
    ...
    L2 -- "No (default)" --> Full
    L2 -- "Yes" --> Cls["Level 2 — call cheap model as<br/>simple/complex classifier (max_completion_tokens:1)"]
    Cls -- "simple" --> Mini
...
- **Level 1 (heuristic)** runs entirely in the APIM policy with zero added round-trips. It
  ...
  length to `autoRouteLengthThreshold` (default 2000 chars) with a ± `autoRouteAmbiguousBand`
  (default 1500) dead-zone. If no user turn can be isolated it falls back to the full text.
  Those numbers look large for "what a developer typed" because they are not: VS Code packs
  editor context *into* the last user message, so a 78-character question routinely measures
  700+. At the original 500/200 every real VS Code turn landed in `long` and took the full
  model, which is why `auto` appeared to save nothing before the retune.
- **Level 2 (classifier)** is **off by default** (`autoRouteClassifierEnabled=false`) because it
  costs an extra model call and needs a mini deployment, but the pilots enable it. Note the
  bands and the classifier are effectively a **package**: with the classifier off, the widened
  ambiguous band just sends VS Code traffic to the full model, so the retune only pays off when
  the classifier is on. When on,
  *only* ambiguous-band prompts trigger a single `max_completion_tokens:1` call to the cheap deployment that
...
flowchart LR
    ...
    APIMin -- "llm-emit-token-metric (platform-captured usage,<br/>streaming AND non-streaming)" --> AI
    APIMin --> Backend["Foundry / AOAI<br/>returns completion + usage{}"]
    Backend --> APIMout["APIM outbound policy"]
...
> **Streaming note.** Copilot CLI and VS Code calls are streaming (`stream:true`). Token metrics
> come from the built-in `llm-emit-token-metric` policy, which the platform evaluates for streamed
> and non-streamed responses alike — verified on both clouds (#126). The inbound policy still
> injects `stream_options.include_usage=true` into streaming chat-completions bodies, because the
> backend only reports usage on the final chunk when asked; `/responses` reports usage natively in
> its terminal event and is left untouched.
>
> This replaced a hand-rolled outbound `emit-metric` pair that read `context.Response.Body`. That
> approach could only parse a JSON body, so it was **completely silent on streamed responses** —
> and since most real traffic streams, token dashboards materially undercounted usage until the
> migration.
...
| `Prompt Tokens` / `Completion Tokens` / `Total Tokens` | inbound (`llm-emit-token-metric`), streaming + non-streaming | platform-captured `usage` | same |
| `Prompt Cached Tokens`, `Completion Reasoning Tokens`, audio + prediction variants | same | platform-captured breakdown | same |
| `copilot_byok_prompt_tokens` / `_completion_tokens`‡ | outbound, **Anthropic requests only** | `usage.input_tokens` / `usage.output_tokens` | same |
...
‡ `copilot_byok_prompt_tokens` / `_completion_tokens` are a **narrow carve-out**, not the general
token metric. `llm-emit-token-metric` supports the **Anthropic Messages** schema only on APIM **v2
tiers**, and v2 is unavailable in Azure Government — so Claude traffic would otherwise be unmetered.
The hand-rolled pair is therefore retained for **Anthropic requests only** (native `/v1/messages` on
the `/anthropic` route); OpenAI-shaped traffic is counted by the built-in policy, so nothing is
double-counted. **Limitation:** an outbound body read cannot see a
streamed response, so a *streaming* native `/v1/messages` call is unmetered until this route can run
on a v2 tier.
...
| extend
    model = tostring(customDimensions["deployment_name"]),
    kind  = iff(name == "Prompt Tokens", "prompt", "completion")
...
| Per-call prompt/completion tokens | ✅ `llm-emit-token-metric` — but with **stock dimensions only**, so no per-developer or per-model slice | ✅ same policy, configured with **our** dimensions (see rows below) — covers streaming *and* non-streaming |
...
|---|---|---|---|
| `byok-standard` | 60 | 100,000 | 50,000 |
| `byok-power` | 120 | 200,000 | 200,000 |

`byok-standard` is sized so a normal interactive VS Code Copilot Chat session does not
throttle: Chat packs editor context into every request (typically 13-15k tokens), so a
20,000 TPM ceiling 429s on the second turn in a minute. 100,000 TPM allows roughly 6-7
turns per minute. These are per-developer *ceilings*, not reservations — they intentionally
oversubscribe the shared model deployment capacity (`modelCapacity`).
...
- **Phase 2b — token-spend alert.** An Azure Monitor **log alert** over the existing
  `copilot.byok` token metrics (`Prompt Tokens` + `Completion Tokens`,
