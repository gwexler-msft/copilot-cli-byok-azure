...
flowchart LR
    ...
    subgraph EdgeReg["Network edge — self-serve onboarding (PLANNED #64)"]
        REG["Register app (ACA external + Entra Easy Auth)<br/>user-assigned MI · provisions per-dev subscription<br/>+ writes client config (VS Code or CLI)"]
    end

    subgraph VNet["Customer Azure VNet 10.60.0.0/16 (private)"]
        APIM["AI Gateway<br/>(Azure API Management — internal VNet mode, classic Developer SKU)<br/>GenAI policies · authMode: subscriptionKey (default) | jwt<br/>validate creds, token-limit + token-metrics,<br/>strip creds, inject MI token, load-balance + route by model"]
        DISC["copilot-byok-discovery API<br/>path /discovery — restricted product 'byok-discovery'<br/>GET /v1/models only · smoke + admins only<br/>(authMode=subscriptionKey + deployFoundry only)"]
    ...
    REG -. "PLANNED: provision per-dev subscription via ARM control plane (MI auth)" .-> APIM
    ...
    DISC -- "GET /openai/v1/models (MI auth, strip caller creds)" --> FOUNDRY
...
> implemented" boxes.
>
> The **`Register app`** box (top-left, same `PLANNED` style) is the self-serve onboarding layer
> tracked in [`#64`](https://github.com/gwexler_microsoft/copilot-cli-byok-azure/issues/64). It
> sits on the network edge (external ACA + Entra Easy Auth, no VNet injection), provisions each
> developer's APIM subscription through the **public ARM control plane** with a least-privilege
> managed identity, and writes the resulting client config to the laptop — `COPILOT_PROVIDER_*`
> for the **CLI** or `chatLanguageModels.json` for **VS Code**. See
> [Self-serve developer onboarding](#self-serve-developer-onboarding-planned--64).
>
> The **`copilot-byok-discovery` API** is also rendered grey because it is restricted-access:
> only the `smoke` CI runner and explicitly named admin keys are scoped to its dedicated
> `byok-discovery` APIM product. Per-developer chat keys (`dev1`, `dev2`, tier subscriptions)
> are not in that product and receive `401` from APIM if they hit `/discovery`. See
> [Model discovery](#model-discovery--separate-api-behind-a-restricted-product-61) for why
> it lives on its own API instead of as an operation on the foundry/aoai inference APIs.
>
...
sequenceDiagram
    ...
    Note over Disk: VS Code → writes chatLanguageModels.json<br/>(APIM host + key + api-key requestHeaders)<br/>CLI → sets COPILOT_PROVIDER_* env (base URL + key)<br/>both merge, do not clobber
...
- **Prompt sizing / content scanning** (`autoPromptText`): walks both `messages[].content`
  and `input[].content`, accepting `text`, `input_text`, and `output_text` parts. The
  auto-route classifier (`auto` model) works identically against either wire format.
...
| **VS Code 1.122+ Custom Endpoint** | Either or both — `chat-completions` and/or `responses` | Defaults to `Authorization: Bearer <apiKey>`; for this gateway override via `requestHeaders: { "api-key": "<APIM_SUBSCRIPTION_KEY>" }` so APIM native subscription-key validation accepts it. Works without GitHub sign-in. | Set `reasoningEffortFormat` to match `apiType`. Ready-made model registration JSON in [`samples/vscode/`](../samples/vscode/). See [deployment-guide → Option C](deployment-guide.md#option-c--vs-code-via-custom-endpoint-byok-provider). |
...
### Model discovery — separate API behind a restricted product (`#61`)

`GET /v1/models` is **not** an operation on the inference APIs. It lives on a dedicated
**`copilot-byok-discovery`** API published at path **`/discovery`**, which in turn is the only
API attached to a dedicated **`byok-discovery`** APIM product. Standard tier subscriptions
(`dev1` / `dev2` / `byok-standard` / `byok-power`) are not in that product, so the same
per-developer key that runs chat completions **cannot** list models — listing is an
explicit admin-only / smoke-runner capability. Only deployed when both `authMode == 'subscriptionKey'`
and `deployFoundry == true`.

```
GET https://<apim>/discovery/v1/models     <- frontend
    api-key: <byok-discovery-subscription-key>

→ rewrite-uri /openai/v1/models            <- API-level policy
...
→ GET https://<foundry>/openai/v1/models   <- backend (OpenAI-compatible /v1/models surface)
```

**Why it is a separate API and product (not an operation on `copilot-byok-foundry`)**

- **Policy compatibility.** The foundry API's API-level policy parses the request body for the
  `model` field and returns `400` when missing. Every `GET /v1/models` would 400 there.
  Lifting model discovery onto its own API removes the foundry body-parsing policy from the
  request path entirely — no operation-level override drift.
- **Auditable access boundary.** "Who can list models?" becomes a *product membership*
  question that is visible in the APIM portal: a developer's subscription is either in the
  `byok-discovery` product or it is not. No allowlist embedded in policy XML to keep in sync.
- **Telemetry hygiene.** Discovery calls do not run `emit-metric`, do not parse a `model`
  body, and do not run the token-cost guard. They never appear in
  `copilot_byok_request` / `copilot_byok_prompt_tokens` / `copilot_byok_completion_tokens`,
  so per-developer billing and rate-limit-by-key counters are not polluted by model-list
  polling.
- **Independent throttle.** The `byok-discovery` product carries its own modest
  `rate-limit-by-key` (default **30 calls/min** per subscription) and `quota-by-key`
  (default **5,000 calls / 30 days**). It deliberately has **no** `azure-openai-token-limit`
  — listing models has no token cost. Tunable via the Bicep params
  `discoveryCallsPerMinute` / `discoveryMonthlyCallQuota`.

**Issuing a discovery key**

The `smoke` runner subscription (created automatically when `deployTestSubscriptions=true`)
is the only key shipped in the default `testSubscriptions` array, and it is scoped *only* to
the `byok-discovery` product — it cannot run chat. To grant a human admin the same
discovery surface, add another entry to `testSubscriptions` (or create a one-off APIM
subscription out of band) with `product: 'byok-discovery'`:

```jsonc
"testSubscriptions": { "value": [
  { "name": "dev1",        "product": "byok-standard"  },
  { "name": "dev2",        "product": "byok-power"     },
  { "name": "smoke",       "product": "byok-discovery" },
  { "name": "admin-gwex",  "product": "byok-discovery" }   // <- adds an admin key
] }
```

> **`authMode=jwt` note.** Discovery is `subscriptionKey`-only today. A jwt variant would
> need a parallel `byok-discovery-policy-jwt.xml` matching the split used by the foundry
> policy (`byok-foundry-policy.xml` vs `byok-foundry-policy-subkey.xml`). Not implemented yet
> because jwt mode is itself opt-in and the byok-discovery product gives us the same "only
> selected callers" property natively without involving Entra.
...
- **App Insights → Live Metrics blade — always empty.** APIM's Application Insights logger
  pushes telemetry through the *classic ingestion path* (instrumentation key → breeze
  ingestion endpoint). Live Metrics is a separate real-time stream that uses the
  **QuickPulse** endpoint and requires the **connection string + SDK-based instrumentation**.
  No SDK-instrumented app exists in this stack (the gateway is the only emitter), so Live
  Metrics has no source and stays empty. To populate it you would have to add a Function /
  App Service / container with the App Insights SDK pointing at the same App Insights
  resource. The `requests`, `dependencies`, and `customMetrics` tables — which feed every
  workbook tile and the KQL files in `monitoring/kql/` — **are** populated within 1-5 min.
...
> **Discovery has its own throttles.** The `byok-discovery` product (model-listing
> only — see [Model discovery](#model-discovery--separate-api-behind-a-restricted-product-61))
> deliberately omits the token-cost guard and runs a much smaller burst limit
> (default 30 calls/min) and quota (5,000 / 30 days). Listing models has no token cost, so
> the AI-cost guard does not apply; the small burst is just there to keep a runaway client
> from hammering the gateway. Tunable via `discoveryCallsPerMinute` /
> `discoveryMonthlyCallQuota` Bicep params.
