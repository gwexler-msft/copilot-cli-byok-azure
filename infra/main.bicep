...
@description('AOAI deployment capacity (TPM units of 1000). 1 unit = 1,000 TPM. This is the SHARED backend pool; the APIM product-tier TPM values are per-developer ceilings that intentionally oversubscribe it (they are limits, not reservations). Default 250 suits a small pilot; raise it as concurrent developer count grows, and check regional model quota first.')
...
param apimExposedModelName string = 'gpt-5.6-sol'
...
@description('Comma-separated model names/prefixes (case-insensitive) that the DEFAULT /openai route transparently routes to the COMMERCIAL Foundry backend instead of the private Foundry, minting a cross-tenant token for it. This is the sentinel that replaced the retired parallel commercial route, and it is what lets the Copilot CLI (azure provider, which can ONLY reach /openai — it discards any base-URL path) use commercial-only models (e.g. gpt-5.6-luna). Requires deployFoundryCommercial=true + the foundryCommercial* settings for the target(s). Empty (default) = INERT: the commercial named values always exist as placeholders, so the policy is valid whether or not commercial is deployed.')
param commercialModels string = ''

// ---- Model API-type capability maps (issue #119 Phase 1 / #122) ----------------------------
// Discovered by scripts/discover-model-types.{ps1,sh}: a compact JSON object mapping each model
// name to the API surfaces it supports (chat/completions, completions, embeddings, responses,
// messages), e.g. {"gpt-5.6-sol":["chat/completions","responses"]}. Phase 2 (#120) policy branches on
// this instead of a static prefix list. Empty ('{}' placeholder) = inert: no model classified, so
// the policy keeps its legacy path-only behaviour. Written to the CI param files by the discovery
// script (authoritative source; a provision applies them). One map per route/cloud.
@description('<FOUNDRY_MODEL_TYPES> Discovered model->API-types map for the DEFAULT Foundry route (/openai), as a compact JSON string. Produced by scripts/discover-model-types. Empty => inert "{}".')
param foundryModelTypes string = ''

@description('<AOAI_MODEL_TYPES> Discovered model->API-types map for the AOAI route (/aoai), compact JSON. Empty => inert "{}".')
param aoaiModelTypes string = ''

@description('<COMMERCIAL_MODEL_TYPES> Discovered model->API-types map for COMMERCIAL Foundry models (read by the /anthropic route policy), compact JSON. Empty => inert "{}".')
param foundryCommercialModelTypes string = ''
...
@description('Deploy the self-healing DNS reconciler Container Apps Job (caj-proxydns) that repoints proxy.byok.internal on drift. Default true (pilots). Requires the pre-baked proxy-dns-reconciler image in the shared ACR (build-runner-image.yml bakes it into the PERSISTENT pilot ACR). Set FALSE on EPHEMERAL envs whose ACR is recreated empty each provision (e.g. dev): the ACA Job validates the image manifest AT PROVISION TIME, so a missing custom image fails the whole provision (MANIFEST_UNKNOWN) — and short-lived envs do not need drift self-healing. Same class as useAcrRunnerImage=false on dev.')
param deployProxyDnsReconciler bool = true
...
// ---------------------------------------------------------------------------------------------
// Native Anthropic Messages route (opt-in; PARALLEL to the default /openai route).
// Adds a SEPARATE APIM API (default path /anthropic) whose ONLY differences from /openai are the
// credential header ('x-api-key' instead of 'api-key') and the wire format it accepts (native
// Anthropic Messages instead of OpenAI chat/completions). It exists because APIM validates the
// subscription key from the header declared on the API BEFORE any policy runs, and an API can
// declare only one such header — so a COPILOT_PROVIDER_TYPE=anthropic client, which always sends
// 'x-api-key', cannot authenticate against /openai no matter what the policy does.
// Same subscription key, same product tier, same quotas and telemetry: this is what makes
// type=anthropic clients meterable per developer key. Backend is still chosen by the
// {{commercial-models}} sentinel (#118), so this is NOT a second backend.
// ---------------------------------------------------------------------------------------------

@description('Deploy the native Anthropic Messages route (new APIM API at anthropicApiPath + product linkage). Default false = nothing is created and Anthropic-speaking clients have no front door. Purely additive: the existing /openai route is untouched.')
param deployAnthropicRoute bool = false

@description('APIM API name for the native Anthropic route. Must be unique within the APIM instance.')
param anthropicApiName string = 'copilot-byok-anthropic'

@description('APIM path segment for the native Anthropic route. Must NOT collide with "openai" or "aoai". Clients set COPILOT_PROVIDER_TYPE=anthropic and COPILOT_PROVIDER_BASE_URL=https://<gateway>/<this> (the anthropic provider PRESERVES the path and appends /v1/messages).')
param anthropicApiPath string = 'anthropic'

// ---------------------------------------------------------------------------------------------
// Commercial Foundry BACKEND (opt-in). The commercial Foundry endpoint is reached over the public
// internet (egress leaves the VNet via the NAT gateway) and is selected per-request by the
// {{commercial-models}} sentinel (#118) on the DEFAULT /openai route and on /anthropic. There is
// no separate API surface: the '/openai-commercial' route was retired once the sentinel made it
// redundant, so clients use one base URL and the gateway picks the backend under the wire.
// ---------------------------------------------------------------------------------------------

@description('Deploy the Commercial Foundry BACKEND (foundry-commercial backend + foundry-commercial-* named values + APIM egress allow rule). Default false = nothing commercial is created and the private Foundry is the only backend. Opt in AND supply foundryCommercialBaseUrl (+ auth + egress) to activate, then list the models in commercialModels so the sentinel routes them.')
...
@description('<COMMERCIAL_API_VERSION> api-version injected on commercial deployment-scoped paths. Confirm it supports the commercial deployment(s) = <COMMERCIAL_DEPLOYMENT_NAMES>.')
...
// ---- Commercial Anthropic/Claude models (issue #116) ---------------------------------------
// Claude models in Foundry speak the ANTHROPIC MESSAGES API (/v1/messages), not OpenAI
// chat/completions. When a caller requests one of these models on the commercial route, APIM
// (a) translates OpenAI chat/completions <-> Anthropic Messages (the shim, on /v1/chat/completions)
// or (b) forwards a native Anthropic body as-is (the /v1/messages operation). Empty list => the
// entire Anthropic path is inert (nothing matches), so this is fully opt-in.
@description('<COMMERCIAL_ANTHROPIC_MODELS> Comma-separated model names or lowercase prefixes on the commercial route that use the Anthropic Messages API instead of OpenAI chat/completions (e.g. "claude-,anthropic-"). Empty (default) disables all Anthropic handling. See issue #116.')
param foundryCommercialAnthropicModels string = ''

@description('<COMMERCIAL_ANTHROPIC_VERSION> Value APIM sends in the anthropic-version header to the commercial Claude backend. TODO(#116): confirm against the live Foundry Claude deployment. Default 2023-06-01.')
param foundryCommercialAnthropicVersion string = '2023-06-01'

@description('<COMMERCIAL_ANTHROPIC_PATH> Backend path APIM rewrites Anthropic requests to on the commercial Foundry endpoint. TODO(#116): confirm the exact Messages path (Hosted-on-Azure vs Anthropic-infrastructure). Default /v1/messages.')
param foundryCommercialAnthropicPath string = '/v1/messages'
...
param miniExposedModelName string = 'gpt-5.6-luna'

@description('Additional standalone model deployments on the primary Foundry account, independent of the full/cheap auto-routing tiers (for example gpt-5.6-terra). Empty (default) = none. Each element: { name, version, sku, capacity, exposedName, raiPolicyName? }. Deploy only where the region hosts each model + SKU.')
...
@description('Auto-routing Level 1: prompt-length threshold (characters of the LAST USER TURN). Shorter non-coding prompts lean to mini, longer to the full model. Sized for real clients, not for what a developer types: VS Code packs editor context into the last user message, so a 78-char question routinely measures 700+. At the old 500 default every real VS Code turn read as long and took the full model.')
param autoRouteLengthThreshold int = 2000

@description('Auto-routing Level 1: half-width (chars) of the ambiguous band around the threshold. Prompts within [threshold-band, threshold+band) are ambiguous and fall through to Level 2 (or the full model if the classifier is off). Set 0 for a hard threshold with no ambiguous band. Note this pairs with autoRouteClassifierEnabled: with the classifier OFF a wide band just sends ambiguous traffic to the full model, so widening it only pays off when the classifier is on.')
param autoRouteAmbiguousBand int = 1500

@description('Auto-routing Level 2: enable the classifier-model call for ambiguous prompts. Off = ambiguous prompts go to the full model (zero added latency). On = a max_completion_tokens:1 call to the mini deployment decides simple/complex (adds one round-trip on the ambiguous band only). Defaults OFF because it costs an extra call and requires deployMiniModel; both pilots turn it ON. Its tokens are metered as copilot_byok_classifier_tokens but CANNOT be charged against the tier TPM (#128).')
...
@description('KV-backed runner-secret toggle (#58/#86). When true (and deployGhRunner=true), the runner Job sources its auth secret from the dedicated runner Key Vault (runner-kv) via a Key Vault reference resolved by the runner UAMI: `gh-app-key` in app mode, `gh-pat` in pat mode. This keeps the credential out of azd state and out of the Job body. BOOTSTRAP IS SINGLE-PASS since #86: runner-identity -> runner-kv (vault + RBAC + secret seeded from ghRunnerPat/ghAppPrivateKey) -> gh-runner (Job with the KV reference). Rotation thereafter is a single `az keyvault secret set` + a Job secret re-resolve (see scripts/rotate-runner-pat-breakglass.ps1) with no re-provision, identical in both clouds. Leave false only for throwaway envs that prefer the inline secret.')
...
var effectiveRegisterAppImage = empty(registerAppImage) ? 'mcr.microsoft.com/dotnet/samples:aspnetapp' : registerAppImage
...
module apimNamedValues 'modules/apim-named-values.bicep' = {
  ...
  params: {
    ...
    // Anthropic/Claude (issue #116). Models list gated on deployFoundryCommercial so an off route
    // keeps the inert '__none__' placeholder; version/path always flow (harmless defaults).
    foundryCommercialAnthropicModels: deployFoundryCommercial ? foundryCommercialAnthropicModels : ''
    foundryCommercialAnthropicVersion: foundryCommercialAnthropicVersion
    foundryCommercialAnthropicPath: foundryCommercialAnthropicPath
    // Model API-type capability maps (issue #119 Phase 1 / #122). Discovered by
    // scripts/discover-model-types and written to the CI param files; '{}' when unset. The
    // commercial map is gated on deployFoundryCommercial so an off route keeps the inert '{}'.
    foundryModelTypes: foundryModelTypes
    aoaiModelTypes: aoaiModelTypes
    foundryCommercialModelTypes: deployFoundryCommercial ? foundryCommercialModelTypes : ''
  }
}
...
// Native Anthropic Messages API (path anthropicApiPath, default 'anthropic'). SEPARATE from the
// routes above, which are untouched. Opt-in via deployAnthropicRoute. Reuses the SAME named
// values, backends and product tiers as /openai — it differs only in the caller credential header
// (x-api-key) and the wire format it accepts. See modules/apim-anthropic-api.bicep for why a
// second API is unavoidable.
module apimAnthropicApi 'modules/apim-anthropic-api.bicep' = if (deployAnthropicRoute) {
  name: 'apim-anthropic-api'
  scope: rg
  params: {
    apimName: apim.outputs.apimName
    // serviceUrl fallback only; the policy's set-backend-service always wins. Anthropic models are
    // commercial-hosted, so the commercial base URL is the sensible fallback when it is supplied.
    fallbackBaseUrl: foundryCommercialBaseUrl
    apiName: anthropicApiName
    apiPath: anthropicApiPath
    authMode: authMode
    namedValueIds: apimNamedValues.outputs.namedValueIds
  }
  dependsOn: [
...
module apimProducts 'modules/apim-products.bicep' = if (authMode == 'subscriptionKey' && deployTestSubscriptions) {
  ...
  params: {
    ...
    apiNames: concat(
      ...
      // Link the Anthropic route into every tier so ONE per-developer subscription key is valid on
      // both /openai and /anthropic. Without this link the key is rejected on the new route.
      deployAnthropicRoute ? [ anthropicApiName ] : []
    )
  }
...
// the runner-kv module's outputs. The name rule mirrors register-kv: <=24 chars, starts with a
// letter, alphanumeric. `environment().suffixes.keyvaultDns` is cloud-correct (.vault.azure.net
// vs .vault.usgovcloudapi.net).
...
// #86 — runner bootstrap ordering: runnerIdentity -> runnerKv -> ghRunner.
// The UAMI is declared on its own so the vault (which needs its principal id for the Secrets
// User role) can be created and SEEDED before the ACA Job is written. Container Apps
// fetch-validates a `keyVaultUrl` secret reference at Job create/update time, so without this
// ordering a fresh provision with ghRunnerSecretFromKeyVault=true deadlocks: the Job fails,
// gh-runner yields no outputs, and the vault it was waiting on is never created.
module runnerIdentity 'modules/runner-identity.bicep' = if (deployGhRunner) {
...
module ghRunner 'modules/gh-runner.bicep' = if (deployGhRunner) {
  name: 'gh-runner'
  scope: rg
  // The Job's Key Vault secret reference is fetch-validated on write, so the vault, the seeded
  // secret and the runner UAMI's Secrets User role must all be in place first.
  dependsOn: [ runnerKv ]
  params: {
    envName: envName
    suffix: suffix
    ...
    runnerUamiId: deployGhRunner ? runnerIdentity.outputs.uamiId : ''
    #disable-next-line BCP318 // guarded by the module's own if (deployGhRunner)
    runnerUamiPrincipalId: deployGhRunner ? runnerIdentity.outputs.uamiPrincipalId : ''
...
// Dedicated RBAC-mode Key Vault for the runner auth secret (#58 — KV-backed rotation, #86 —
// single-pass bootstrap). Created whenever the runner is deployed, regardless of whether
// ghRunnerSecretFromKeyVault is on yet. It also SEEDS the active mode's secret from the same
// credential CI already injects, so the Job's Key Vault reference resolves on the very first
// provision. The runner UAMI gets Secrets User (read); the deploying principal (when known)
// gets Secrets Officer (write) for the break-glass rotation helper.
@description('Lock the runner Key Vault down: publicNetworkAccess=Disabled + a Private Endpoint in snet-pe (+ the privatelink.vaultcore zone). The VNet-integrated runner env resolves the secret reference over the PE. Secret SEEDING still works while locked (ARM is a trusted service), so this does not affect provisioning; only ad-hoc `az keyvault secret set` from outside the VNet is blocked. Default false.')
...
module runnerKv 'modules/runner-kv.bicep' = if (deployGhRunner) {
  ...
  params: {
    ...
    #disable-next-line BCP318 // guarded by the module's own if (deployGhRunner)
    runnerUamiPrincipalId: deployGhRunner ? runnerIdentity.outputs.uamiPrincipalId : ''
    // Seed only the ACTIVE auth mode's secret; an empty value is a no-op (leaves any existing
    // secret untouched) so a provision without the credential can't blank a live runner.
    seedSecretName: ghRunnerAuthMode == 'app' ? 'gh-app-key' : 'gh-pat'
    seedSecretValue: ghRunnerAuthMode == 'app' ? ghAppPrivateKey : ghRunnerPat
    deployerPrincipalId: deployerPrincipalId
...
module ghRunnerRbac 'modules/runner-rbac.bicep' = if (deployGhRunner) {
  ...
  params: {
    #disable-next-line BCP318 // guarded by the module's own if (deployGhRunner)
    runnerPrincipalId: deployGhRunner ? runnerIdentity.outputs.uamiPrincipalId : ''
    #disable-next-line BCP318 // guarded by deployFoundry
    foundryAccountName: deployFoundry ? foundry.outputs.foundryAccountName : ''
    #disable-next-line BCP318 // guarded by deployAoai
    aoaiAccountName: deployAoai ? aoai.outputs.aoaiAccountName : ''
...
var deployByokInternalDns = deployFoundry && deployFoundrySubkeyProxy
...
module registerApp 'modules/register-app.bicep' = if (deployRegisterApp) {
  ...
  params: {
    ...
    // Same sentinel list the gateway policy uses, so the register app only ever advertises models
    // this deployment can actually route.
    commercialModels: deployFoundryCommercial ? commercialModels : ''
    ...
    registerImage: effectiveRegisterAppImage
    #disable-next-line BCP318 // guarded by deployRegisterApp
...
// Shared byok.internal private DNS zone + VNet link (#117), created when EITHER in-VNet proxy is
// deployed. Each proxy module adds only its OWN A record under this zone (referenced as existing).
module byokInternalDns 'modules/byok-internal-dns.bicep' = if (deployByokInternalDns) {
  name: 'byok-internal-dns'
  scope: rg
  params: {
    proxyDnsZoneName: 'byok.internal'
    vnetId: network.outputs.vnetId
    envName: envName
    suffix: suffix
  }
...
module proxyDnsReconcileJob 'modules/proxy-dns-reconcile-job.bicep' = if (deployFoundry && deployFoundrySubkeyProxy && deployProxyDnsReconciler && deployGhRunner && deploySharedAcr) {
...
#disable-next-line BCP318 // guarded by deployGhRunner
output ghRunnerUamiName string = deployGhRunner ? runnerIdentity.outputs.uamiName : ''
...
#disable-next-line BCP318 // guarded by deployGhRunner
output ghRunnerUamiClientId string = deployGhRunner ? runnerIdentity.outputs.uamiClientId : ''

@description('Self-hosted GitHub Actions runner UAMI principal ID (empty when deployGhRunner=false).')
#disable-next-line BCP318 // guarded by deployGhRunner
output ghRunnerUamiPrincipalId string = deployGhRunner ? runnerIdentity.outputs.uamiPrincipalId : ''
...
#disable-next-line BCP318 // guarded by deployGhRunner
output ghRunnerFicSubjects array = deployGhRunner ? runnerIdentity.outputs.ghFicSubjects : []
