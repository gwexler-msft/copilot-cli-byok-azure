...
@description('Additional standalone model deployments on this account, independent of the full/cheap auto-routing tiers (for example gpt-5.6-terra). Empty = none. Each element: { name, version, sku, capacity, exposedName, raiPolicyName? }. raiPolicyName is optional and defaults to the primary raiPolicyName (already authored below); a DISTINCT custom policy must already be authored by the primary/mini or be a Microsoft.* built-in. Deploy only where the region hosts each model + SKU.')
...
// Optional additional standalone deployments (e.g. Commercial-only models reached via the
// cross-cloud commercial route). @batchSize(1) + dependsOn on the primary + mini serialize the
// concurrent PUTs on the same account so they do not race (AccountProvisioningStateInvalid).
@batchSize(1)
resource additionalDeployments 'Microsoft.CognitiveServices/accounts/deployments@2024-10-01' = [for m in additionalModels: {
  parent: foundry
  name: m.exposedName
  sku: {
    name: m.sku
  ...
  properties: {
    ...
    raiPolicyName: m.?raiPolicyName ?? raiPolicyName
  }
  dependsOn: [modelDeployment, miniDeployment]
}]
