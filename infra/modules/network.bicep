...
resource nsgVm 'Microsoft.Network/networkSecurityGroups@2024-01-01' = if (deployTestVm && restrictVmEgress) {
  ...
  properties: {
    securityRules: [
      ...
      {
        // Log Analytics / App Insights query data plane. In Government that API is unreachable
        // from outside the VNet, so the test VM is the only place scripts/probe-observability.ps1
        // can run; without this rule the locked-down VM cannot reach its own diagnostic endpoints.
