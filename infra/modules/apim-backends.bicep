...
// AND the default /openai policy (commercialModels branch, #118) target it via set-backend-service
// backend-id="{{foundry-commercial-backend-id}}". Created UNCONDITIONALLY (placeholder URL when the
// commercial route is off) because APIM validates set-backend-service backend existence at
// policy-apply time: the default policy references it on EVERY env, so the backend must always
// exist even where deployFoundryCommercial=false (it stays inert — commercialModels is empty there,
// so the policy never routes to it). Circuit breaker is reused (same toggle).
resource foundryCommercialBackend 'Microsoft.ApiManagement/service/backends@2024-06-01-preview' = {
  parent: apim
  name: 'foundry-commercial'
  properties: {
    protocol: 'http'
    url: empty(foundryCommercialUrl) ? 'https://unset.invalid' : foundryCommercialUrl
    circuitBreaker: enableCircuitBreaker ? { rules: breakerRules } : null
  }
}
