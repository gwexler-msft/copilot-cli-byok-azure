...
@description('Private DNS zone (VNet-linked, owned by byok-internal-dns.bicep) that hosts the stable proxy hostname. An A record for proxyHostLabel is (re)set to the ACI CURRENT private IP on every provision, so clients use a fixed FQDN and never chase the dynamic ACI IP.')
...
// it, so rather than expose the changing IP we give clients a fixed FQDN whose A record points at
// the ACI's CURRENT IP. The zone + VNet link are owned by byok-internal-dns.bicep (#117); this
// module only (re)sets its OWN A record on every provision. Low TTL so any move converges fast.
