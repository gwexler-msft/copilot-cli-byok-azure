...
// ------------------------------------------------------------------------------------
// Runner identity (issues #53 phase 2, #86). The UAMI and its GitHub OIDC federated
// credentials are declared in `runner-identity.bicep` and passed in here, so the runner
// Key Vault (which needs the principal id for its Secrets User role) can be created
// BEFORE this module — the ACA Job fetch-validates its `keyVaultUrl` secret reference at
// create/update time, so the vault + secret + role must already exist. See the header of
// runner-identity.bicep for the full ordering rationale.
// ------------------------------------------------------------------------------------
...
@description('Principal (object) id of the runner user-assigned managed identity (runner-identity module output `uamiPrincipalId`). Used for the AcrPull role assignment on the shared ACR.')
...
resource runnerAcrPull 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!empty(acrName)) {
  scope: sharedAcr
  name: guid(sharedAcr.id, runnerUamiId, acrPullRoleId)
  properties: {
