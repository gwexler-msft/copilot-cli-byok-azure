...
resource appiLogger 'Microsoft.ApiManagement/service/loggers@2024-05-01' = {
  ...
  properties: {
    ...
    // Use the CONNECTION STRING (not a bare instrumentationKey) so APIM's Application Insights
    // client resolves the correct per-cloud endpoints. The connection string carries LiveEndpoint,
    // which is https://live.applicationinsights.azure.us/ in Azure Government -- required for the
    // Live Metrics (QuickPulse) stream to connect. With only an instrumentation key APIM falls back
    // to the Commercial live endpoint and Gov Live Metrics shows "couldn't connect to your
    // application" (regular ikey ingestion still works, so requests/customMetrics are unaffected).
    credentials: { connectionString: appInsightsConnectionString }
    isBuffered: true
  }
}
...
resource diag 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = {
  ...
  properties: {
    ...
    // GatewayLogs belongs to NO category group, so `categoryGroup: 'allLogs'` silently enables
    // nothing and ApiManagementGatewayLogs stays empty forever while metrics keep flowing. The
    // category has to be named explicitly. Government exposes only GatewayLogs; the extra
    // Commercial categories (GatewayLlmLogs, GatewayMCPLogs, WebSocketConnectionLogs,
    // DeveloperPortalAuditLogs) would fail a Gov deployment, so this stays on the common set.
    logs: [
