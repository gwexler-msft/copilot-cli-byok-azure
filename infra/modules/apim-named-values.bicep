...
@description('Comma-separated model names/prefixes the default /openai policy routes to the commercial Foundry backend (cross-cloud). Empty => "__none__" placeholder (inert).')
...
// ---- Commercial Anthropic/Claude named values (issue #116) ---------------------------------
@description('Comma-separated model names/prefixes on the commercial route that use the Anthropic Messages API (shim + native /v1/messages). Empty => Anthropic handling disabled (inert "__none__" sentinel). Placeholder <COMMERCIAL_ANTHROPIC_MODELS>.')
param foundryCommercialAnthropicModels string = ''

@description('anthropic-version header value sent to the commercial Claude backend. TODO(#116) confirm against the live deployment. Default 2023-06-01.')
param foundryCommercialAnthropicVersion string = '2023-06-01'

@description('Backend path Anthropic requests are rewritten to on the commercial endpoint. TODO(#116) confirm the exact Messages path. Default /v1/messages.')
param foundryCommercialAnthropicPath string = '/v1/messages'

// ---- Model API-type capability maps (issue #119 Phase 1 / #122) ----------------------------
@description('Discovered model->API-types map for the DEFAULT Foundry route (/openai), compact JSON. Empty => inert "{}". Placeholder <FOUNDRY_MODEL_TYPES>.')
param foundryModelTypes string = ''

@description('Discovered model->API-types map for the AOAI route (/aoai), compact JSON. Empty => inert "{}". Placeholder <AOAI_MODEL_TYPES>.')
param aoaiModelTypes string = ''

@description('Discovered model->API-types map for COMMERCIAL Foundry models (read by the /anthropic route policy), compact JSON. Empty => inert "{}". Placeholder <COMMERCIAL_MODEL_TYPES>.')
...
// Models the DEFAULT /openai policy routes to the commercial Foundry backend (cross-cloud). The
// placeholder '__none__' keeps the branch inert when unset; the policy prefix/exact-matches the
// requested model against this list. Mirrors aoai-pinned-models.
resource nvCommercialModels 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'commercial-models'
  properties: {
    displayName: 'commercial-models'
    value: empty(commercialModels) ? '__none__' : commercialModels
    secret: false
  }
}
...
// ---- Commercial Anthropic/Claude named values (issue #116) ---------------------------------
// The commercial policy prefix-matches the requested model against this comma-separated list to
// decide OpenAI-vs-Anthropic handling. The '__none__' sentinel (when empty) matches no model, so
// the Anthropic shim + native /v1/messages translation stay fully inert until models are listed.
resource nvFoundryCommercialAnthropicModels 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'foundry-commercial-anthropic-models'
  properties: {
    displayName: 'foundry-commercial-anthropic-models'
    value: empty(foundryCommercialAnthropicModels) ? '__none__' : foundryCommercialAnthropicModels
    secret: false
  }
}

resource nvFoundryCommercialAnthropicVersion 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'foundry-commercial-anthropic-version'
  properties: {
    displayName: 'foundry-commercial-anthropic-version'
    value: empty(foundryCommercialAnthropicVersion) ? '2023-06-01' : foundryCommercialAnthropicVersion
    secret: false
  }
}

resource nvFoundryCommercialAnthropicPath 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'foundry-commercial-anthropic-path'
  properties: {
    displayName: 'foundry-commercial-anthropic-path'
    value: empty(foundryCommercialAnthropicPath) ? '/v1/messages' : foundryCommercialAnthropicPath
    secret: false
  }
}
...
// messages). Phase 2 (#120) policy branches on this instead of a static prefix list. '{}' when
// unset = inert (no model classified; policy keeps its legacy path-only behaviour). One per route.
resource nvFoundryModelTypes 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'foundry-model-types'
  properties: {
    displayName: 'foundry-model-types'
    value: empty(foundryModelTypes) ? '{}' : foundryModelTypes
    secret: false
  }
}

resource nvAoaiModelTypes 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'aoai-model-types'
  properties: {
    displayName: 'aoai-model-types'
    value: empty(aoaiModelTypes) ? '{}' : aoaiModelTypes
    secret: false
  }
}

resource nvFoundryCommercialModelTypes 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'foundry-commercial-model-types'
  properties: {
    displayName: 'foundry-commercial-model-types'
    value: empty(foundryCommercialModelTypes) ? '{}' : foundryCommercialModelTypes
    secret: false
  }
}
...
output namedValueIds array = [
  ...
  nvFoundryCommercialAnthropicModels.id
  nvFoundryCommercialAnthropicVersion.id
  nvFoundryCommercialAnthropicPath.id
  nvFoundryModelTypes.id
  nvAoaiModelTypes.id
  nvFoundryCommercialModelTypes.id
]
