...
// passes to the runner Job.
//
// BOOTSTRAP (#86): this module runs BEFORE gh-runner and SEEDS the active auth secret itself
// (`seedSecretName`/`seedSecretValue`, fed from the CI repo secret), so a single provision now
// produces vault -> secret -> RBAC -> Job-with-KV-reference. The seed write goes through ARM,
// which is a TRUSTED first-party service, so it succeeds even while the vault is locked to
// publicNetworkAccess=Disabled (this is the same path the break-glass rotation script falls
// back to). Seeding is SKIPPED when `seedSecretValue` is empty, so a provision run without the
// credential in its environment leaves the existing secret untouched rather than blanking it.
...
@description('Principal ID of the runner UAMI (runner-identity module output). Granted Key Vault Secrets User (read) so the Container Apps Job resolves the gh-pat/gh-app-key secret reference.')
param runnerUamiPrincipalId string

@description('Name of the secret to seed with `seedSecretValue`: `gh-pat` (pat auth mode) or `gh-app-key` (app auth mode). Only the ACTIVE mode\'s secret is seeded.')
@allowed([ 'gh-pat', 'gh-app-key' ])
param seedSecretName string = 'gh-pat'

@description('Value written to `seedSecretName` on every provision (#86 — sourced from the CI repo secret GH_RUNNER_PAT / GH_APP_PRIVATE_KEY). EMPTY = skip the write entirely and leave any existing secret value alone, so a provision without the credential in its environment cannot blank a live runner credential.')
@secure()
param seedSecretValue string = ''
...
// Seed/refresh the active auth secret (#86). Skipped when the value is empty so an environment
// missing the credential never overwrites a live one. Written via ARM (trusted service), so this
// works with the vault locked to publicNetworkAccess=Disabled and no in-VNet host required.
resource seededSecret 'Microsoft.KeyVault/vaults/secrets@2023-07-01' = if (!empty(seedSecretValue)) {
  parent: kv
  name: seedSecretName
  properties: {
    value: seedSecretValue
  }
}
