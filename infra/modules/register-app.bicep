...
@description('Auto-route sentinel model id offered to VS Code in the rendered chatLanguageModels.json. Empty omits it. Binds to Byok:AutoModelId.')
param autoModelId string = 'auto'

@description('Comma-separated commercial-only model ids reachable via the commercialModels sentinel on /openai. Pass the same value as the gateway commercialModels setting, or empty when the commercial backend is not deployed - otherwise the register app advertises models VS Code can never resolve. Binds to Byok:CommercialModels.')
...
@description('Minutes of inactivity before the register page wipes the displayed key and redirects to Easy Auth logout (#136). Browser-side UX only - the enforcement is easyAuthSessionLifetime below. 0 disables the idle timer.')
param sessionIdleTimeoutMinutes int = 15

@description('Absolute Easy Auth session lifetime (HH:MM:SS) applied as authConfig login.cookieExpiration with convention=FixedTime (#136). The client cannot bypass this. Keep it LOOSER than sessionIdleTimeoutMinutes: matching them would sign out a developer who is actively working, mid-flow.')
param easyAuthSessionLifetime string = '01:00:00'

@description('Extra OIDC parameters sent to Entra on every login, e.g. [\'prompt=login\'] (#136). EMPTY by default. prompt=login was tried and MEASURED USELESS: A/B on comm-dev + gov-dev showed sign-out already sticks without it (returning demands a fresh login), and on a device holding a Primary Refresh Token it is satisfied by the PRT so it does not force credentials either - the one case it was added for. It only cost an extra prompt on every sign-in. Forcing re-credentialing on a PRT-joined device is a Conditional Access sign-in-frequency policy, not an app setting.')
param easyAuthLoginParameters array = []
      ...
      containers: [
        {
          ...
          env: [
            ...
            {
              name: 'Byok__EntraLoginHost'
              value: entraLoginHost
            }
            {
            ...
            {
              name: 'Byok__SessionIdleTimeoutMinutes'
              value: string(sessionIdleTimeoutMinutes)
            }
            {
              name: 'Byok__AutoModelId'
              value: autoModelId
            }
            {
              name: 'Byok__CommercialModels'
              value: commercialModels
            }
            {
...
resource auth 'Microsoft.App/containerApps/authConfigs@2024-10-02-preview' = if (easyAuthEnabled) {
  ...
  properties: {
    ...
    login: {
      // Absolute cap on the session; the in-page idle timer is UX only and can be bypassed.
      cookieExpiration: {
        convention: 'FixedTime'
        timeToExpiration: easyAuthSessionLifetime
      }
    }
    identityProviders: {
      azureActiveDirectory: {
        enabled: true
        login: {
          loginParameters: easyAuthLoginParameters
        }
        registration: {
