...
deployment-scoped). Token metrics (`Prompt Tokens` / `Completion Tokens` / `Total Tokens`,
from APIM's built-in `llm-emit-token-metric`) emit for both surfaces, and for streamed as
well as non-streamed responses.
